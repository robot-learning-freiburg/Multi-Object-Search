...
from src.igibson.env_functions import BaseFunctions
from src.igibson.mapping_module import MappingModule
from src.igibson.multi_object_task import MultiObjectTask
...
class MultiObjectEnv(BaseFunctions):
    ...
    def reset(self):
        ...
        if self.mapping.aux_prob_counter > 4 and self.episode_counter > 25 and np.mean(
            ...
            print("------------------>>>>>>>> NEW EPISODIC PROB: ", self.aux_episodic_prob, " for env : ",
                  self.last_scene_id)  # , " with env succ-rate: ",succ_rate_mean)
