...
from src.general.SB3.torch_layers_MOD import MlpExtractor,MlpExtractor_Aux
from stable_baselines3.common.torch_layers import (
    BaseFeaturesExtractor,
    CombinedExtractor,
    FlattenExtractor,
    #MlpExtractor,
    #MlpExtractor_Aux,#Added by Fabi
    NatureCNN,
    create_mlp,
)
...
class ActorCriticPolicy(BasePolicy):
    ...
    def _build_mlp_extractor(self) -> None:
        ...
        #substract 5 as this is the proprioceptive input.
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        #----------------------------------------------------
        #Added by Fabi
        #if(self.aux_pred_dim == 1):
        #self.aux_net =nn.Linear(self.mlp_aux_extractor.latent_dim_aux, self.aux_pred_dim)
        self.aux_net_angle = nn.Sequential(nn.Linear(self.mlp_aux_extractor.latent_dim_aux, self.aux_pred_dim), nn.Tanh())
        #self.aux_net_distance = nn.Sequential(nn.Linear(self.mlp_aux_extractor.latent_dim_aux, 1), nn.ReLU())
        ...
        #self.aux_net.apply(init_weights)    
        #--------------------------------------
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        features = self.extract_features(obs) # 512
        
        latent_pi, latent_vf = self.mlp_extractor(features)
        #auxilliary prediction based on map only (cut of proprioceptive obs input)

        #cut out the propriceptive observation wihtout the ground truth angle first two
        
        #print("obs_prop",obs_prop[0],obs_prop.size(),obs['task_obs'][0])
        

        #print(aux_inp)
        #print("shape:",obs['task_obs'].size())
        #print("after:",obs['task_obs'][:,self.cut_out_aux_head::].size())
        latent_aux = self.mlp_aux_extractor(th.cat([obs['task_obs'][:,self.cut_out_aux_head::],features[:,128::]], dim=1).float())
        aux_angle = self.aux_net_angle(latent_aux)
        #aux_dist = self.aux_net_distance(latent_aux)
        #latent_aux = self.mlp_aux_extractor(features)
        ...
        return actions, values, log_prob,aux_angle#,aux_dist
    ...
    def evaluate_actions(self, obs: th.Tensor, actions: th.Tensor) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        #print("unobserved:",obs['task_obs'][0,:13])
        #print("Aux only input:",aux_inp[0,:11])
        latent_aux = self.mlp_aux_extractor(th.cat([obs['task_obs'][:,self.cut_out_aux_head::],features[:,128::]], dim=1).float())
        distribution = self._get_action_dist_from_latent(latent_pi)
        log_prob = distribution.log_prob(actions)
        values = self.value_net(latent_vf)
        ...
        return values, log_prob, distribution.entropy(), aux_angle#, aux_dist
    ...
    def get_distribution(self, obs: th.Tensor) -> Distribution:
        ...
        #print("aux input:",aux_inp[0,:13])
        #print("SHAPE:",aux_inp.size())
        #print("shape:",obs['task_obs'].size())
        #print("after:",obs['task_obs'][:,self.cut_out_aux_head::].size())
        latent_aux = self.mlp_aux_extractor(th.cat([obs['task_obs'][:,self.cut_out_aux_head::],features[:,128::]], dim=1).float())
        aux_angle = self.aux_net_angle(latent_aux)
        ...
        return self._get_action_dist_from_latent(latent_pi), aux_angle#, aux_dist
